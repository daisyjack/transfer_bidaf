...
def evaluate_all(my_arg, pr=True):
    ...
    batch_getter_lst = []
    if my_arg == 0:
        pernam_batch_getter = BatchGetter('data/eng_dev.txt', 'PER_NAM', 1, False)
        batch_getter_lst.append(pernam_batch_getter)

        pernom_batch_getter = BatchGetter('data/eng_dev.txt', 'PER_NOM', 1, False)
        batch_getter_lst.append(pernom_batch_getter)
    ...
    embedding_layer.load_state_dict(torch.load(model_dir+'/early_embedding_layer.pkl'))
    att_layer.load_state_dict(torch.load(model_dir+'/early_att_layer.pkl'))
    model_out_layer.load_state_dict(torch.load(model_dir+'/early_model_out_layer.pkl'))
